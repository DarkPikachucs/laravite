<x-phy70::layouts.master>
  ...
  // แนวทางการพัฒนา (ดึงจากกิจกรรม)
  $activityGuidelines = collect($activities)
  ->pluck('guideline')
  ->filter()->map(fn ($x) => trim($x))->unique()->values();
  ...
  ? array_values(array_filter($fyList, fn ($y) => (string) $y <= $endYear)) : []; if (count($coveredYears) <=1) {
    $durationText=$endYear !=='' ? 'ปีงบประมาณ พ.ศ. ' . $endYear : '—' ; } else {
    $durationText='ทุกปีงบประมาณ ตั้งแต่ พ.ศ. ' . $coveredYears[0] . ' ถึง พ.ศ. ' . end($coveredYears) . ' (รวม ' .
    count($coveredYears) . ' ปี)' ; } @endphp <style>
    .brief-page {
    --ink: #1a1a1a;
    --ink-soft: #444;
    --line: #333;
    ...
    <div class="brief-page">
      ...
      <!-- The form sheet -->
      <div class="brief-sheet" id="brief-sheet-content">
        ...
        <div class="brief-line">
          ...
          <span class="val">{{ $activityGuidelines->isNotEmpty() ? $activityGuidelines->implode(' · ') : '—' }}</span>
        </div>
        <div class="brief-line">
        ...
        <table class="brief-table">
          ...
          <tbody>
            ...
            <tr class="act-line">
              ...
              <td class="brief-detail is-block"><b class="brief-money">{{ number_format((float) ($act['budget'] ?? 0),
                  2) }}</b> บาท</td>
            </tr>
            <tr class="act-line">
      ...
      function exportToWord(elementId, filename = 'document') {
          ...
          var styles = `
              <style>
                  ...
                  body { font-family: 'Sarabun', 'Cordia New', 'TH SarabunPSK', sans-serif; font-size: 11pt; color: #000; }
                  table { width: 100%; border-collapse: collapse; margin-top: 15px; }
                  th, td { border: 1px solid #000; padding: 8px; vertical-align: top; text-align: left; font-size: 11pt; }
                  ...
                  .brief-heading { text-align: center; font-size: 12pt; font-weight: bold; margin-bottom: 20px; }
                  .brief-line { margin-bottom: 10px; }
                  .lbl { font-weight: bold; }
                  .col-topic { font-weight: bold; width: 35%; }
                  .num { font-weight: bold; }
                  .brief-meta { margin-top: 20px; font-size: 10pt; color: #666; text-align: right; }
