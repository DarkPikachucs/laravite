<x-phy70::layouts.master>
    ...
    <style>
        ...
        .title { font-size: 24px; font-weight: 600; font-family: 'Prompt', sans-serif; }
        .glass-card {
            background: var(--bg-surface); backdrop-filter: blur(16px);
            border: 1px solid var(--border-glow); border-radius: 24px;
            padding: 40px; box-shadow: 0 15px 40px rgba(0, 0, 0, 0.3); margin-bottom: 28px;
        }
        .section-header { border-bottom: 1px solid rgba(255, 255, 255, 0.08); padding-bottom: 12px; margin-bottom: 20px; }
        .section-title { font-size: 18px; font-weight: 600; color: var(--secondary); font-family: 'Prompt', sans-serif; }
        .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px; }
        @media (max-width: 600px) { .info-grid { grid-template-columns: 1fr; } }
        .info-item { margin-bottom: 20px; }
        .info-label { font-size: 13px; color: var(--text-muted); font-weight: 500; margin-bottom: 6px; font-family: 'Prompt', sans-serif; text-transform: uppercase; }
        .info-val {
        ...
        .bg-glow {
            ...
            top: 10%; left: 5%; pointer-events: none; z-index: 0;
        }
    </style>
    ...
    <div class="show-container">
        ...
        <div class="glass-card" style="background: linear-gradient(135deg, rgba(99, 102, 241, 0.1) 0%, rgba(6, 182, 212, 0.1) 100%);">
            ...
            <h1 style="font-size: 24px; font-weight: 600; font-family: 'Prompt', sans-serif; color: var(--text-main); line-height: 1.4; margin-bottom: 12px;">
            ...
            <div style="font-size: 13.5px; color: var(--text-muted); display: flex; gap: 20px; flex-wrap: wrap;">
                ...
                <span>ส่งวันที่: <strong style="font-family: monospace;">{{ $proposal->created_at->format('Y-m-d H:i') }}</strong></span>
            </div>
        </div>
        ...
        <div class="glass-card">
                ...
                <div class="info-val" style="padding: 0; background: transparent; border: none;">
                    @foreach($proposal->kpis as $kpi)
                        @if(isset($kpi['selected']) && $kpi['selected'])
                            <div style="background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.05); padding: 12px 16px; border-radius: 10px; margin-bottom: 8px;">
                                <div style="font-weight: 500; margin-bottom: 8px; color: var(--secondary);">{{ $kpi['name'] }}</div>
                                <div style="display: flex; gap: 16px; font-size: 13px; flex-wrap: wrap;">
                                    @if(isset($kpi['targets']) && is_array($kpi['targets']))
                                        @foreach($kpi['targets'] as $i => $t)
                                            <div style="background: rgba(255,255,255,0.05); padding: 4px 8px; border-radius: 6px;">
                                                <span style="color: var(--text-muted);">ปี {{ 2568 + $i }}:</span> 
                                                <span>{{ $t ?: '-' }}</span>
                                            </div>
                                        @endforeach
                                    @endif
            ...
            <div class="info-item">
                ...
                <div class="info-val">
                    ...
                    @if(!empty($proposal->target_district)) ➔ อำเภอ{{ is_array($proposal->target_district) ? implode(', ', $proposal->target_district) : $proposal->target_district }} @endif
                    @if(!empty($proposal->target_subdistrict)) ➔ ตำบล{{ is_array($proposal->target_subdistrict) ? implode(', ', $proposal->target_subdistrict) : $proposal->target_subdistrict }} @endif
                </div>
            </div>
                ...
                <div class="info-item" style="margin-top: 24px; border-top: 1px solid rgba(255, 255, 255, 0.05); padding-top: 20px;">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 10px; margin-top: 10px;">
                        @foreach($proposal->attachments as $attachment)
                            <a href="{{ $attachment['path'] }}" target="_blank" class="info-val" style="display: flex; align-items: center; gap: 10px; text-decoration: none; background: rgba(99, 102, 241, 0.08);">
                        ...
                        <div class="info-item" style="margin-bottom: 12px;">
                            ...
                            <div class="info-val">
                                @if(isset($act['target_province']))
                                    ...
                                    @if(!empty($act['target_district'])) ➔ อำเภอ{{ is_array($act['target_district']) ? implode(', ', $act['target_district']) : $act['target_district'] }} @endif
                                    @if(!empty($act['target_subdistrict'])) ➔ ตำบล{{ is_array($act['target_subdistrict']) ? implode(', ', $act['target_subdistrict']) : $act['target_subdistrict'] }} @endif
                                @else
                                    {{ $act['target_area'] ?? '-' }}
                                @endif
                            </div>
                ...
                <div class="info-val">
                    <ul style="padding-left: 20px;">
                        @foreach($proposal->documents as $doc)
                            ...
                            @endphp
                            <li><a href="{{ Storage::url(preg_replace('~^/storage/~', '', $docPath)) }}" target="_blank" style="color: var(--secondary); text-decoration: underline;">{{ $docName }}</a></li>
                        @endforeach
                    </ul>
                </div>
            </div>
