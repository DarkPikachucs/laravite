    ...
    <div class="dashboard-container">
        ...
        <div id="orgs" class="tab-content active glass-card">
            ...
            <table class="custom-table">
                <thead><tr><th>ID</th><th>ชื่อหน่วยงาน</th><th>จำนวนผู้ใช้</th><th>จำนวนโครงการ</th><th>การจัดการ</th></tr></thead>
                <tbody>
                    @foreach($organizations as $org)
                    <tr>
                        <td>{{ $org->id }}</td>
                        <td>
                            <form action="{{ route('phy70.superadmin.org.update', $org->id) }}" method="POST" style="display:flex; gap:8px;">
                                @csrf
                        ...
                        <td>
                            <form action="{{ route('phy70.superadmin.org.delete', $org->id) }}" method="POST" onsubmit="return confirm('ยืนยันการลบหน่วยงานนี้? ข้อมูลที่เกี่ยวข้องจะถูกลบทั้งหมด');">
                                @csrf @method('DELETE')
        ...
        <div id="users" class="tab-content glass-card">
            ...
            <table class="custom-table">
                <thead><tr><th>ชื่อ</th><th>อีเมล</th><th>หน่วยงาน</th><th>สิทธิ์</th><th>การจัดการ</th></tr></thead>
                <tbody>
                    @foreach($users as $u)
                    <tr>
                        <td>{{ $u->name }}</td>
                        ...
                        <td>
                            <form action="{{ route('phy70.superadmin.user.role', $u->id) }}" method="POST" style="display:flex; gap:8px;">
                                @csrf
                        ...
                        <td>
                            <form action="{{ route('phy70.superadmin.user.delete', $u->id) }}" method="POST" onsubmit="return confirm('ยืนยันการลบบัญชีนี้?');">
                                @csrf @method('DELETE')
                                <button type="submit" class="btn-danger">ลบ</button>
                            </form>
                        </td>
        ...
        <div id="proposals" class="tab-content glass-card">
            ...
            <table class="custom-table">
                <thead><tr><th>ชื่อโครงการ</th><th>หน่วยงาน</th><th>ผู้เสนอ</th><th>สถานะ</th><th>การจัดการ</th></tr></thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $p->user->name ?? '-' }}</td>
                        <td>
                            <form action="{{ route('phy70.superadmin.proposal.status', $p->id) }}" method="POST" style="display:flex; gap:8px;">
                                @csrf
                        ...
                        <td style="display: flex; gap: 8px;">
                            <a href="{{ route('phy70.proposal.show', $p->id) }}" class="btn-action" style="text-decoration: none; padding: 6px 10px;">เปิดดู</a>
                            <a href="{{ route('phy70.superadmin.proposal.edit', $p->id) }}" class="btn-secondary" style="text-decoration: none; padding: 6px 10px; color: var(--secondary); border-color: rgba(6, 182, 212, 0.4);">แก้ไข</a>
                            <form action="{{ route('phy70.superadmin.proposal.delete', $p->id) }}" method="POST" onsubmit="return confirm('ยืนยันการลบโครงการนี้?');">
                                @csrf @method('DELETE')
